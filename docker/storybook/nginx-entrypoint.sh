...
# Copia exacta del entrypoint ya probado en HCE-MF-PLATFORM
# (docker/nginx-entrypoint.sh) -- mismo mecanismo de deteccion de
# certificados que el API Gateway (ag-pruebas-ag). NGINX_TYPE=spa fijo
# aca (Storybook es un build estatico con fallback de rutas, no un
