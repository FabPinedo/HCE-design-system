...
import type { SearchModeOption } from "../../src/molecules/SearchComboInput/SearchComboInput";
...
const MOCK_DIAGNOSTICOS: SearchOption[] = [
  {
    value: 1,
    label: "Infección aguda de las vías respiratorias superiores",
    secondary: "J06.9",
  },
  { value: 2, label: "Neumonía, no especificada", secondary: "J18.9" },
  {
    value: 3,
    label: "Gastroenteritis y colitis de origen infeccioso",
    secondary: "A09",
  },
  {
    value: 4,
    label: "Apendicitis aguda, sin mención de peritonitis",
    secondary: "K35.8",
  },
  { value: 5, label: "Hipertensión esencial (primaria)", secondary: "I10" },
  {
    value: 6,
    label: "Diabetes mellitus tipo 2 sin complicaciones",
    secondary: "E11.9",
  },
  {
    value: 7,
    label: "Infección de las vías urinarias, sitio no especificado",
    secondary: "N39.0",
  },
...
function filterOptions(query: string, mode: SearchMode): SearchOption[] {
  ...
  if (mode === "cie_code") {
    return MOCK_DIAGNOSTICOS.filter((o) =>
      o.secondary?.toLowerCase().startsWith(q),
    );
  }
    ...
    return (
      <Box sx={{ maxWidth: 600, p: 3 }}>
        ...
        {selected && (
          <Box
            sx={{
              ...
              bgcolor: "#f5fcec",
              borderRadius: "8px",
              border: "1px solid #89c93d",
            }}
          >
            <Box
              sx={{ fontFamily: "monospace", fontSize: 11, color: "#6B7280" }}
            >
              Seleccionado:
            </Box>
            <Box sx={{ fontWeight: 600, fontSize: "0.875rem", mt: 0.5 }}>
              {selected.label}
            </Box>
            {selected.secondary && (
              <Box
                sx={{ fontFamily: "monospace", fontSize: 12, color: "#003d96" }}
              >
                CIE-10: {selected.secondary}
              </Box>
            )}
          </Box>
        )}
        ...
        >
          ...
          <br />
          Modo "CIE-10": escribe el código (ej: "J06", "A09")
          <br />
          Modo "Por nombre": escribe parte del diagnóstico
        </Box>
      </Box>
    );
  },
...
export const Loading: Story = {
  ...
  render: () => (
    <Box sx={{ maxWidth: 600, p: 3 }}>
      <SearchComboInput label="Motivo de ingreso" value="neumon" loading />
    </Box>
  ),
};
...
export const Disabled: Story = {
  ...
  render: () => (
    <Box sx={{ maxWidth: 600, p: 3 }}>
      <SearchComboInput label="Motivo de ingreso" value="Neumonía" disabled />
    </Box>
  ),
};
